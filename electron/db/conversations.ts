...
export function listConversations(projectId: string): Conversation[] {
  ...
  return db
    .prepare(
      "SELECT id, project_id as projectId, title, created_at as createdAt, updated_at as updatedAt FROM conversations WHERE project_id = ? ORDER BY updated_at DESC"
    )
...
export function getConversation(id: string): Conversation | undefined {
  ...
  return db
    .prepare(
      "SELECT id, project_id as projectId, title, created_at as createdAt, updated_at as updatedAt FROM conversations WHERE id = ?"
    )
