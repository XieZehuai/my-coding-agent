...
#### Scenario: Skills warmed from DB on first access per session

- **WHEN** a runtime is freshly created (e.g., after app restart) and `chat-service.sendChatMessage` runs for the first time in this app session
- **THEN** the service layer SHALL hydrate the runtime by reading skill names via `getConversationSkillNames(convId)` and calling `runtime.addSkill(name)` for each, gated by a `runtime.warmed` flag so warming is idempotent
