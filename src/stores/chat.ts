...
export const useChatStore = defineStore('chat', () => {
  ...
  function endToolCall(id: string, result: string) {
    const seg = streamingSegments.value.find(
      (s) => s.type === 'tool_call' && s.toolCall.id === id
    )
  ...
  function finishStreaming() {
    const hasContent = streamingSegments.value.some((s) =>
      s.type === 'text' ? s.content.length > 0 : true
    )
