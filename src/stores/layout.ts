...
export const useLayoutStore = defineStore("layout", () => {
  ...
  function clampOnMount() {
    ...
    inputHeight.value = Math.min(
      inputHeight.value,
      Math.max(LAYOUT_CONSTRAINTS.input.min, vh * LAYOUT_CONSTRAINTS.input.maxVh)
    );
    devPanelWidth.value = Math.min(
      devPanelWidth.value,
      Math.max(LAYOUT_CONSTRAINTS.devPanel.min, vw * LAYOUT_CONSTRAINTS.devPanel.maxVw)
    );
  }
