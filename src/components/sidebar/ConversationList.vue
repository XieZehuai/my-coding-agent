<template>
  <div class="sidebar-section conversations-section">
    ...
    <div class="section-list conversation-list">
      ...
      <div v-if="convStore.conversations.length === 0 && projectStore.selectedProjectId" class="empty-hint">
        Click + to start
      </div>
...
async function handleUndo() {
  if (contextMenu.convId) {
    const r = await convStore.undoConversation(contextMenu.convId);
...
async function finishRename(id: string) {
  if (renamingId.value === id) {
    const t = renameValue.value.trim() || "未命名";
    await convStore.renameConversation(id, t);
