...
const inputResize = useResizable({
  value: computed({
    ...
    set: (v) => { layoutStore.inputHeight = v },
  }),
  min: 60,
...
.chat-header { display: flex; align-items: center; justify-content: space-between; padding: 12px 20px; border-bottom: 1px solid var(--border); background: var(--header-bg); min-height: 52px; }
.chat-title { font-size: 16px; font-weight: 600; color: var(--text-primary); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.header-actions { display: flex; gap: 8px; align-items: center; }
.trust-toggle { background: none; border: 1px solid var(--border); color: var(--text-secondary); font-size: 16px; cursor: pointer; padding: 4px 10px; border-radius: 6px; transition: all 0.15s; }
.trust-toggle:hover { background: var(--bg-hover); }
.trust-toggle.active { background: var(--warning); border-color: var(--warning); color: var(--warning-text); }
.input-wrapper { flex-shrink: 0; overflow: hidden; }
</style>
